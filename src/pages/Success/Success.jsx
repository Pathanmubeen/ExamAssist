    ...
    <AnimateProvider className="flex flex-col space-y-10 md:max-w-xl md:mx-auto">
      ...
      {allQuestion.length > 0 ? (
        allQuestion.map((question, index) => {
          const answerItem = userAnswer.find((ua) => ua.question.question === question.question) || {};
          return (
            <div key={index} className="bg-white p-4 rounded-lg shadow-md mb-6">
              <div className="flex flex-col space-y-2">
                ...
                <div className="flex justify-between text-neutral-800 font-semibold">
                  ...
                  <span className="font-normal">{answerItem.answer || "Skipped"}</span>
                </div>

                <div className="flex justify-between text-neutral-800 font-semibold">
                  <span>Correct answer:</span>
                  <span className="font-normal">{question.correct_answer}</span>
                </div>

                <div
                  className={`text-xl font-semibold ${
                    answerItem.isCorrect ? "text-green-500" : answerItem.answer ? "text-red-500" : "text-yellow-500"
                  }`}
                >
                  {answerItem.isCorrect ? "Correct" : answerItem.answer ? "Incorrect" : "Skipped"}
                </div>
                ...
                <div className="flex justify-between text-neutral-800 font-semibold">
                  ...
                  <span className="font-normal">
                    {questionTimes[index] !== undefined
                      ? questionTimes[index] === 'N/A'
                        ? 'N/A'
